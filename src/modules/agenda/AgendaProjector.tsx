...
export const AgendaProjector: React.FC<{ config: AgendaConfig; isActive: boolean }> = ({
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-between p-6 md:p-12 max-w-7xl mx-auto z-10 select-none overflow-hidden">
      {/* Header */}
      <div className="text-center space-y-2.5 mb-4 shrink-0">
      ...
      {/* Timeline Grid with padding to prevent clipping of shadows, glows, and scale */}
      <div className="w-full flex-1 min-h-0 overflow-y-auto p-3 md:p-5">
          ...
          {activeProfile.items.map((item, index) => {
            ...
            return (
              <div
                ...
                className={`relative flex flex-col justify-between p-6 rounded-2xl transition-all duration-300 cursor-pointer border ${
                  isCurrent
                    ? 'bg-gradient-to-br from-purple-900/90 to-slate-900/95 border-purple-400 shadow-2xl shadow-purple-600/50 ring-2 ring-purple-400/60 scale-[1.02] z-10'
                    : 'bg-slate-900/60 hover:bg-slate-800/70 border-white/10 hover:border-white/20'
                }`}
              >
                {/* Card top bar */}
      ...
      {/* Footer Info */}
      <div className="mt-4 text-center text-xs md:text-sm text-slate-400 font-medium shrink-0">
